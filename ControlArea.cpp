...
void ControlArea::initial_condition_3(double h)
{
	for (int i = 0; i < m_cell_Q.size(); ++i)
	{
		m_cell_Q[i] = 0.5*(0.2369268851*function_3(m_cell_X[i] - 0.5*h*0.9061798459) + 0.4786286705*function_3(m_cell_X[i] - 0.5*h*0.5384693101)
			+ 0.5688888889*function_3(m_cell_X[i]) + 0.4786286705*function_3(m_cell_X[i] + 0.5*h*0.5384693101) + 0.2369268851*function_3(m_cell_X[i] + 0.5*h*0.9061798459));
	}
	m_cell_Q_previous = m_cell_Q;
}
...
double ControlArea::function_1(double x)
{
	...
	if ((x <= sqrt(0.3)) && (x >= -sqrt(0.3))) out = sqrt(1 - (10.0 / 3.0)*x*x);
	else out = 0.0;
...
{
	double out = exp(-300.0*x*x);

	return out;
}

double ControlArea::function_3(double x)
{
