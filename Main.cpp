...
int main()
{
	...
	for (time = 0.0; time < TargetTime - dt;)
	{
		...
		//-----------------------------reconstruction test-------------------------------
		std::vector<double> reconst;
		reconst.clear();
		for (int i = 0; i < Rx.size(); ++i)
		{
			reconst.push_back(Rx[i].reconstruct(controlArea->getCell_X()[i]));
			//reconst.push_back(Rx[i].reconstruct(controlArea->getCell_X()[i] - 0.5*dx));
			//reconst.push_back(Rx[i].reconstruct(controlArea->getCell_X()[i] + 0.5*dx));
		}
		//-------------------------------------------------------------------------------
		...
		// solution update
		/*for (int i = nSideGhost; i < (NFX - nSideGhost); ++i)
		{
			Solution[i] = prev_Solution[i] - (dt / dx)*(flux_p->flux(i) - flux_m->flux(i));
		}*/
